var _ = require('underscore'),
    ...
    SqlCmdLineByLineReader = require('./SqlCmdLineByLineReader'),
...
var Utils = (function() {
    ...
    function runQuery(messages, writer, query) {
        ...
        return query(messages).then(function(recordsets) {
...
var RunCommand = (function () {
    ...
    RunCommand.prototype.run = function (messages, writer, args) {
        ...
        var variables = _.extend({}, process.env); // the environment variables can be used as variables in sqlcmd
        var variablesString = args.slice(1).join(' ');
        var variablesRegex = /([^\s"]+)\s*=\s*([^\s]+)/gi;
        while ((match = variablesRegex.exec(variablesString)) !== null) {
        ...
        var deferred = Q.defer(),
            file = args[0],
            invoker = new Invoker(this.db, messages, writer),
            reader = new SqlCmdLineByLineReader(file, variables),
            last = Q(),
            self = this,
            buffer = '';
